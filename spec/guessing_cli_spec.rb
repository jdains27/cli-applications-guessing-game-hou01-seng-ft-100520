...
describe "Guessing CLI" do
  ...
  context 'user inputs' do
   ...
   it "responds to an incorrect guess" do
      allow(self).to receive(:rand).and_return(5)	      allow(self).to receive(:rand).and_return(5)


      expect(self).to receive(:gets).and_return("2")	      expect(self).to receive(:gets).and_return("2")


      expect { run_guessing_game }.to output(/Sorry! The computer guessed 6./).to_stdout	      expect { run_guessing_game }.to output(/Sorry! The computer guessed 5./).to_stdout
    end	    end
